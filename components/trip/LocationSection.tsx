  ...
  return (
    <div className="mb-3">
      ...
      <button
        ...
        className="w-full flex items-center gap-1.5 py-2 px-1 text-right group"
        aria-expanded={open}
      >
        <ChevronRight
          size={16}
          className={`text-navy transition-transform duration-200 shrink-0 ${open ? "rotate-90" : ""}`}
        />
        <Flag code={group.flag} size={18} />
        <span className="font-bold text-navy text-base">{group.label}</span>
        <span className="text-xs text-gray-400">{days.length} ימים</span>
      </button>

      {/* Days list */}
      {open && (
        <div className="mr-2 border border-argentina-light rounded-xl overflow-hidden">
          {days.map((day, i) => (
            <DayRow
              key={day.day}
              tripDay={day}
